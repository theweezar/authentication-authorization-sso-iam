...
test("Admin users cannot create other Admin users", async () => {
    ...
    const userService = new UserService(context.transactionManager, context.userRepository, context.roleRepository, passwordHasher);
    ...
    await assert.rejects(userService.createUser({
        organizationId: organization.organization.id,
        roleId: adminRole.id,
        email: "second-admin@example.test",
        phone: "1000000002",
        password: "SecondAdmin!123",
    }, toActor(createdAdmin)), (error) => {
...
test("Admin users cannot remove other Admin users", async () => {
    ...
    await assert.rejects(userService.deleteUser(secondAdmin.id, toActor(firstAdmin)), (error) => {
        assert.ok(error instanceof AppError);
        assert.equal(error.statusCode, 403);
        return true;
    });
...
test("bulkDeleteUsers rolls back when one deletion is not allowed", async () => {
    ...
    const adminRole = await context.roleRepository.findByName("Admin");
    const memberRole = await context.roleRepository.create({ name: "Member", description: "Standard member role", createdById: "system" });
    assert.ok(superAdmin);
    assert.ok(adminRole);
    ...
    await assert.rejects(userService.bulkDeleteUsers([adminUser.id, memberUser.id], toActor(adminUser)), (error) => {
        assert.ok(error instanceof AppError);
        assert.equal(error.statusCode, 403);
        return true;
    });
