...
test("local strategy fails when credentials are missing", async () => {
  ...
  const authService = new AuthService(
    ...
    new JwtTokenService("test-secret", "1h"),
  );
  const strategy = new LocalStrategy(authService);

  const failure = await new Promise<{ challenge: unknown; status: number | undefined }>((resolve) => {
    strategy.fail = (challenge: unknown, status?: number) => resolve({ challenge, status });
    strategy.authenticate({ body: {} } as never);
  });
...
test("local strategy succeeds with valid credentials", async () => {
  ...
  const authenticatedUser = await new Promise<{ id: string; email: string }>((resolve) => {
    strategy.success = (user: unknown) => resolve(user as { id: string; email: string });
